...
- `data/` es la única capa que habla con MySQL (usa `mysql-connector-python`); expone funciones tipo repositorio (`crear_producto`, `listar_productos`, `actualizar_producto`, `eliminar_producto`) que reflejan el esquema de [02-modelo-de-datos.md](02-modelo-de-datos.md). **Estado** (2026-09-16): ✅ **CRUD completo implementado** (`data/db.py` + `data/productos.py`) y probado contra Aiven en producción (RF1-RF4). `listar_productos`/`obtener_producto` filtran siempre por `activo = 1`; `eliminar_producto` es baja lógica (`UPDATE activo = 0`, nunca `DELETE`).
